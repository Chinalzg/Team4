...
class Admin extends Model
{
    ...
    //通过查询用户名判断用户是否正确登录
     ...
     public function shows(){
         return $data=DB::select("SELECT * from power where id in(SELECT power_id from r_p where role_id in(select id from role where id in(select role_id from u_r where admin_id IN ('".Session::get('admin_id')."'))))");
     }
