        ...
        <div class="Add_Addresss"  >
            ...
            <table>
                <tr>
                    ...
                    <td><input name="" type="text"  @blur="checkaddress"  v-model="registerModel.address" class="Add-text"/><i>（必填)
                            <br><span>@{{msgaddress}}</span>  </i></td>
                    <td><input type="hidden"  v-model="registerModel.id"></td>
                </tr>

                <tr>
                    <td class="label_name">收件人姓名</td><td><input name="" type="text" @blur="checkname"  v-model="registerModel.name" class="Add-text"/><i>（必填）<br><span>@{{ msgname }}</span> </i></td>
                    <td class="label_name">电子邮箱</td><td><input name="" type="text"  v-model="registerModel.email" class="Add-text"/><i>（选填）</i></td>
                </tr>
                <tr>
                    <td class="label_name">邮&nbsp;&nbsp;编</td><td><input name="" @blur="checkpostal"  v-model="registerModel.postal" type="text"  class="Add-text"/><i>（必填）<br><span>@{{ msgpostal }}</span></i></td>
                    <td class="label_name">性&nbsp;&nbsp;别</td><td>
                        <label class="sex"> <input type="radio" name="RadioGroup1" v-model="registerModel.sex" value="0" id="RadioGroup1_0"  class="select"/><span>男</span></label>
                        <label class="sex"><input type="radio" name="RadioGroup1"  v-model="registerModel.sex" value="1" id="RadioGroup1_1" class="select"/><span>女</span></label><i>（选填）</i> </td>
                </tr>
                <tr><td class="label_name">手&nbsp;&nbsp;机</td><td><input name="" type="text"  class="Add-text" @blur="checktel"  v-model="registerModel.tel"/><i>（必填）<br><span>@{{ msgtel }}</span></i></td>
        ...
        var vm = new Vue({
            ...
            methods:{
                ...
                checkpostal:function(){
                    ...
                    if(this.registerModel.postal==""){
                        this.msgpostal="邮编不能为空";
                    }else if(!reg.test(this.registerModel.postal)){
                        this.msgpostal="请输入有效邮编";
                    }else{
                        this.msgpostal="输入正确"
                        return true;
                    }
                ...
                checktel:function(){
                    ...
                    if(this.registerModel.tel==""){
                        this.msgtel="手机号不能为空";
                    }else if(!myreg.test(this.registerModel.tel)){
                        this.msgtel="请输入有效的手机号码";
                    }else{
                        this.msgtel="输入正确"
                        return true;
                ...
                register: function() {
               if (this.checkaddress() && this.checktel() && this.checkpostal() && this.checkname())
               {
                   var vm = this
                   vm.msg = ''
