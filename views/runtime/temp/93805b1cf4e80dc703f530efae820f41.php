<?php /*a:3:{s:75:"D:\phpStudy\PHPTutorial\WWW\views\application\index\view\user\my_atten.html";i:1558679099;s:38:"./static/common/html/index_header.html";i:1559216585;s:43:"./static/common/html/right_person_info.html";i:1558663101;}*/ ?>
<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
    ...
    <div class="content excerpt excerpt-1" style="padding: 15px 15px 11px 0px;">
      ...
      <div class="container">
        <table class="table table-striped" style="width: 100%;">
          ...
          <tbody>
            <?php foreach($myAtten as $k => $v): ?>
              <tr>
                <td>
                  <?php if((empty($v['user_headimg']))): ?>
                  <img src="/static/index/Uploads/headImg/head.png" style="width: 30px;height: 30px;border-radius: 50%;">
                  <?php else: ?>
                  <img src="/static/index/Uploads/headImg/<?php echo htmlentities($v['user_headimg']); ?>" style="width: 30px;height: 30px;border-radius: 50%;">
                  <?php endif; ?>
                  <?php echo htmlentities($v['user_name']); ?>
                </td>
                <td><?php echo htmlentities(date("Y-m-d H:i:s",!is_numeric($v['atten_addtime'])? strtotime($v['atten_addtime']) : $v['atten_addtime'])); ?></td>
                <td>
                  <a href="">查看他</a> |
                  <a href="javascript:void(0);" class="unatten" data-atten_user_id="<?php echo htmlentities($v['atten_user_id']); ?>">取消关注</a>
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
...
<script>
     ...
     * 取消关注事件
     */
    $(".unatten").on('click',function () {
        const atten_user_id = $(this).attr('data-atten_user_id');

        $.post("/index/User/unattenUser",{"article_user_id":atten_user_id},function (jsonMsg) {
